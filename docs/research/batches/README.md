# 연구 배치

이 디렉터리는 같은 데이터·체결·평가 조건으로 실행한 전략 묶음의 규칙, 핵심 결과와 판정을 배치별로 기록한다. 원시 CSV와 메타데이터는 `results/research/`에 둔다.

| 배치 | 범위 |
| --- | --- |
| [1차](01-single-asset-timing.md) | 단일 자산 추세·모멘텀·돌파·계절성·평균회귀 |
| [2차](02-bll-technical-rules.md) | BLL 이동평균과 거래범위 돌파 |
| [3차](03-multi-asset-allocation.md) | GTAA·GEM과 정적 다중자산 배분 |
| [4차](04-paa2.md) | Protective Asset Allocation PAA2 |
| [5차](05-moving-average-vo.md) | 이동평균 단독 운용과 VO 결합 규칙 |
| [6차](06-defensive-allocation.md) | VAA-G4·DAA-G12의 빠른 모멘텀과 카나리 방어 |
| [7차](07-flexible-asset-allocation.md) | 블라인드 탐색으로 찾은 FAA 기본형의 독립 평가 |
