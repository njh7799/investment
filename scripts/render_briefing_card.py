...
def _draw_vo_panel(draw: ImageDraw.ImageDraw, briefing: BriefingLike) -> None:
    ...
    draw.text((570, 620), f"{briefing.volatility:.1%}", font=_font(39, bold=True), fill=TEXT, anchor="rm")
    label, background, foreground = volatility_state(briefing.volatility)
    label_font = _font(22, bold=True)
    bbox = draw.textbbox((0, 0), label, font=label_font)
    width = bbox[2] - bbox[0] + 34
    draw.rounded_rectangle((690 - width, 591, 710, 649), radius=15, fill=background)
    draw.text((700 - width / 2, 620), label, font=label_font, fill=foreground, anchor="mm")
