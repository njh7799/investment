...
from backtests.allocation_models import FAA_ASSETS, build_allocation_weights

MODELS = ("faa7_equal_monthly", "faa_default", "tqqq_hold", "qqq_hold", "vo")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze preregistered default Flexible Asset Allocation")
...
@lru_cache(maxsize=1)
def prepared():
    assets = load_assets(FAA_ASSETS, ROOT)
    index = next(iter(assets.values())).index
    for frame in list(assets.values())[1:]:
        index = index.intersection(frame.index)
    closes = pd.DataFrame({name: assets[name].loc[index, "Close"] for name in FAA_ASSETS}, index=index)
    targets = {model: build_allocation_weights(model, closes) for model in ("faa7_equal_monthly", "faa_default")}
    first_signal = targets["faa_default"].sum(axis=1).ne(0.0).idxmax()
    evaluation_start = index[index.get_loc(first_signal) + 1]
    return assets, closes, targets, evaluation_start
...
def main() -> int:
    ...
    metadata = {
        ...
        "base_fee_rate": 0.001,
        "models": list(MODELS),
        "preregistration": "docs/research/preregistrations/batch-07-faa.yaml",
        "asset_sha256": {name: hashlib.sha256((ROOT / "assets" / f"{name}.csv").read_bytes()).hexdigest() for name in FAA_ASSETS},
    }
