#!/usr/bin/env python3
from __future__ import annotations

import argparse
import hashlib
import json
import sys
from functools import lru_cache
from pathlib import Path

import pandas as pd

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from backtests import build_target_weights, load_assets, load_market_data, run_jordan_public_proxy, run_jordan_tqqq, run_three_percent_rule, run_weight_strategy, summarize

MODELS = (
    "jordan_public_proxy",
    "qqq_hold",
    "jordan_tqqq_full_exit",
    "jordan_tqqq_one_third",
    "jordan_tqqq_two_thirds",
    "tqqq_hold",
    "vo",
    "three_percent",
)
PERIODS = {
    "synthetic_full": (None, None),
    "actual_tqqq_full": ("2010-02-11", None),
    "recent_10y": ("2016-08-01", None),
}
STRESS = {
    "dotcom_bust": ("2000-03-24", "2002-10-09"),
    "global_financial_crisis": ("2007-10-31", "2009-03-09"),
    "covid_crash": ("2020-02-19", "2020-03-23"),
    "2022_bear": ("2021-11-19", "2022-12-28"),
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze preregistered Jordan IXIC -3% rules")
    parser.add_argument("--output", type=Path, required=True)
    return parser.parse_args()


@lru_cache(maxsize=1)
def inputs():
    return load_market_data(ROOT), load_assets(("TLT",), ROOT)["TLT"]


@lru_cache(maxsize=None)
def market_weights(model: str):
    return build_target_weights(model, inputs()[0])


def run(model: str, start=None, end=None, fee=0.001):
    data, tlt = inputs()
    if model == "jordan_public_proxy":
        return run_jordan_public_proxy(data, tlt, start=start, end=end, fee_rate=fee)
    panic_weights = {
        "jordan_tqqq_full_exit": 0.0,
        "jordan_tqqq_one_third": 1.0 / 3.0,
        "jordan_tqqq_two_thirds": 2.0 / 3.0,
    }
    if model in panic_weights:
        return run_jordan_tqqq(data, panic_weight=panic_weights[model], start=start, end=end, fee_rate=fee)
    if model == "three_percent":
        return run_three_percent_rule(data, start=start, end=end, fee_rate=fee)
    prices, weights = market_weights(model)
    return run_weight_strategy(prices, weights, start=start, end=end, fee_rate=fee)


def rolling_rows(years: int) -> list[dict[str, object]]:
    index = inputs()[0].index
    eligible = index[(index >= pd.Timestamp("2010-02-11")) & (index <= index[-1] - pd.DateOffset(years=years))]
    starts = list(pd.Series(eligible, index=eligible).groupby(eligible.to_period("M")).first())
    rows = []
    for model in MODELS:
        metrics = [summarize(run(model, start, min(start + pd.DateOffset(years=years), index[-1]))) for start in starts]
        rows.append({
            "model": model,
            "window_years": years,
            "window_count": len(metrics),
            "median_cagr": float(pd.Series([item["cagr"] for item in metrics]).median()),
            "worst_cagr": float(min(item["cagr"] for item in metrics)),
            "median_mdd": float(pd.Series([item["mdd"] for item in metrics]).median()),
            "worst_mdd": float(min(item["mdd"] for item in metrics)),
            "median_longest_tuw_days": float(pd.Series([item["longest_time_underwater_days"] for item in metrics]).median()),
            "worst_longest_tuw_days": int(max(item["longest_time_underwater_days"] for item in metrics)),
        })
    return rows


def main() -> int:
    args = parse_args()
    args.output.mkdir(parents=True, exist_ok=True)
    period_rows = [{"period": period, "model": model, **summarize(run(model, start, end))} for period, (start, end) in PERIODS.items() for model in MODELS]
    pd.DataFrame(period_rows).to_csv(args.output / "period-summary.csv", index=False, float_format="%.10f")
    stress_rows = [{"period": period, "model": model, **summarize(run(model, start, end))} for period, (start, end) in STRESS.items() for model in MODELS]
    pd.DataFrame(stress_rows).to_csv(args.output / "stress-summary.csv", index=False, float_format="%.10f")
    pd.DataFrame(rolling_rows(5) + rolling_rows(10)).to_csv(args.output / "rolling-summary.csv", index=False, float_format="%.10f")
    fee_rows = [{"fee_rate": fee, "model": model, **summarize(run(model, "2010-02-11", fee=fee))} for fee in (0.001, 0.002, 0.005) for model in MODELS]
    pd.DataFrame(fee_rows).to_csv(args.output / "fee-summary.csv", index=False, float_format="%.10f")
    data, _ = inputs()
    metadata = {
        "generated_through": data.index[-1].date().isoformat(),
        "actual_tqqq_start": "2010-02-11",
        "initial_cash": 100_000,
        "base_fee_rate": 0.001,
        "models": list(MODELS),
        "preregistration": "docs/research/preregistrations/batch-11-jordan-ixic-three-percent.yaml",
        "asset_sha256": {name: hashlib.sha256((ROOT / "assets" / f"{name}.csv").read_bytes()).hexdigest() for name in ("IXIC", "QQQ", "TQQQ", "TLT")},
    }
    (args.output / "metadata.json").write_text(json.dumps(metadata, indent=2, sort_keys=True) + "\n")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
