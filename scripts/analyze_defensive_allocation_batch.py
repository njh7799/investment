...
from backtests.allocation_models import DAA_CANARY, DAA_G12_RISKY, VAA_CASH, VAA_G4_RISKY, build_allocation_weights

MODELS = ("tqqq_hold", "qqq_hold", "vo", "vaa_g4", "daa_g12")
ASSET_NAMES = tuple(dict.fromkeys([*VAA_G4_RISKY, *VAA_CASH, *DAA_G12_RISKY, *DAA_CANARY]))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze preregistered VAA-G4 and DAA-G12 models")
...
@lru_cache(maxsize=1)
def prepared():
    assets = load_assets(ASSET_NAMES, ROOT)
    index = next(iter(assets.values())).index
    for frame in list(assets.values())[1:]:
        index = index.intersection(frame.index)
    closes = pd.DataFrame({name: assets[name].loc[index, "Close"] for name in ASSET_NAMES}, index=index)
...
def run(model: str, start=None, end=None, fee=0.001):
    ...
    return run_portfolio_strategy(prices, target, start=start or COMMON_START, end=end, fee_rate=fee, rebalance_monthly=True)
...
def main() -> int:
    ...
    metadata = {"generated_through": market().index[-1].date().isoformat(), "common_start": COMMON_START, "initial_cash": 100_000, "base_fee_rate": 0.001, "models": list(MODELS), "asset_sha256": {name: hashlib.sha256((ROOT / "assets" / f"{name}.csv").read_bytes()).hexdigest() for name in ASSET_NAMES}}
